...
vec3 pathtrace(vec3 origin, vec3 ray, vec3 light, float t,float time, int sample) {		
	...
	for(int bounce = 0; bounce < MAX_BOUNCES; bounce++) {			
		...
		if(  val.x < t) {			  	
			//calcualte the surface color 
												// w  : material index
												// yz ; uv coordinates
			surfaceColor = mix(texture(textureMaps, vec3(val.y, val.z, val.w  ) ), vec4(1.0), vec4(val.w==255.0)  ).xyz; 
			// surfaceColor = mix(vec4(1,1,0,1), vec4(0.5), vec4(val.w==255.0) ).xyz; 
			...
			origin = hit;	
			ray = uniformlyRandomDirection(time + float(bounce) + float(sample));	
...
{ 
	...
	if(tNearFar.x < tNearFar.y  ) {
		...
		// vec3 x = pathtrace(eyeRay.origin, eyeRay.dir, light, t);

		vec3 color;
        vec3 outColor = vec3(0);
        for(int s=0; s < spp; s++){
        	if (spp != 1){
        		vec3 light = light_position + uniformlyRandomVector(float(s)/100 + 0.01 );
        		color = pathtrace(eyeRay.origin, eyeRay.dir, light, t, float(s)/100 + 0.01,  0);
        	}
        	else{
        		vec3 light = light_position + uniformlyRandomVector(time);
        		color = pathtrace(eyeRay.origin, eyeRay.dir, light, t, time,  0);
        	}
		...
		vFragColor =  vec4(outColor, 1);


		// vFragColor = vec4(0.0);
	}

	// vFragColor = vec4(tNearFar.x < tNearFar.y);
