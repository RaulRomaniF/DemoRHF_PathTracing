class ObjLoader{
	...
	static parseObjText(txt,flipYUV){
		...
		var line,				//Line text from obj file
			...
			cVertIndex = [],			//Cache Vertice array read from obj
			boxMin = [1000, 1000, 1000],			//Final Index Sorted Vertice Array
			boxMax = [-1000, -1000, -1000],			//Final Index Sorted Vertice Array
		...
		while(posB > posA){
			...
			switch(line.charAt(0)){
				...
				case "v":
					...
					switch(line.charAt(1)){
						case " ": 
							...
							var vertex = [parseFloat(itm[0]), parseFloat(itm[1]), parseFloat(itm[2]) ];
							cVert.push(vertex[0] , vertex[1] , vertex[2] ); 
							
							if (vertex[0] < boxMin[0])
								boxMin[0] = vertex[0];

							if (vertex[1] < boxMin[1])
								boxMin[1] = vertex[1];

							if (vertex[2] < boxMin[2])
								boxMin[2] = vertex[2];


							if (vertex[0] > boxMax[0])
								boxMax[0] = vertex[0];

							if (vertex[1] > boxMax[1])
								boxMax[1] = vertex[1];

							if (vertex[2] > boxMax[2])
								boxMax[2] = vertex[2];
				...
				case "f":
					...
					itm.shift();
					var values_txt = itm.shift();
					var item = values_txt.split("/");
					cVertIndex.push(parseInt(item[0]-1));
					// cVertIndex.push(parseInt(item[1]));
					// cVertIndex.push(parseInt(item[2]));
					// cVertIndex.push(parseInt(255));


					values_txt = itm.shift();
					item = values_txt.split("/");
					cVertIndex.push(parseInt(item[0]-1));
					// cVertIndex.push(parseInt(item[1]));
					// cVertIndex.push(parseInt(item[2]));
					// cVertIndex.push(parseInt(255));
		...
		return [cVert, cVertIndex, boxMin, boxMax];		
	}
