...
RayTracingDemo.prototype.Load = function (cb) {
	...
	me.url = "http://" + document.domain + ":" + location.port;


	async.parallel({
		Models: function (callback) {
			async.map({
				CubeModelText: me.url + '/static/models/cube.obj'
			}, LoadTextResource, callback);
		},
		ShaderCode: function (callback) {
			async.map({
				'PathTracer_VSText': me.url + '/static/shaders/pathTracer.vs.glsl',
				'PathTracer_FSText': me.url + '/static/shaders/pathTracer.fs.glsl',
			}, LoadTextResource, callback);
		}
		...
		me.gl.uniform1f( me.gl.getUniformLocation(me.PathTracerProgram, "spp")  , 0);
...
RayTracingDemo.prototype.GenerateRHFInputs = function () {
    ...
    gl.uniform1f(gl.getUniformLocation(this.PathTracerProgram, "spp"), 1);
    ...
    var histoChannelsBase64 = {};	
    histoChannelsBase64.samples = spp;
    for (var s = 1; s <= spp; ++s)
    {
        gl.disable(gl.DEPTH_TEST);
        gl.clear(gl.COLOR_BUFFER_BIT);
        gl.uniform1f (gl.getUniformLocation(this.PathTracerProgram, "time"), parseFloat(s) / 100.0 + 0.01);
        ...
        histoChannelsBase64["channel" + s] = base64;
    }


    //************* SOCKETS ***************//
	var socket = io.connect(this.url);
	socket.emit( 'histogram', histoChannelsBase64 );

	console.log("histogram was sended sucessfully");

	//************* START SEND IMAGE MADE OF SAMPLES *****************/

    gl.uniform1f(gl.getUniformLocation(this.PathTracerProgram, "spp"), spp);
    gl.disable(gl.DEPTH_TEST);
    gl.clear(gl.COLOR_BUFFER_BIT);
  ...
  for (var j=0; j<imageData.height; j++)
	...
	console.log("image was sended sucessfully");
	//************* END SEND IMAGE MADE OF SAMPLES *****************/
...
RayTracingDemo.prototype.DrawPixelsBuffer = function () {
 ...
 //    });
	...
	// var obj = {};
	// for (var i = 1; i <= 3; i++) {
	// 	obj["channel" + i] = base64;
	// }
	// socket.emit( 'mouse', obj );

	// var startTime = new Date();
	// var endTime = new Date();
	// var timeDiff = endTime - startTime;
	// console.log("ellpased time js: " + timeDiff);
